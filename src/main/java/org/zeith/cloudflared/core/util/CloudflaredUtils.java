...
public class CloudflaredUtils {
    ...
    static CompletableFuture<Integer> download(String url, File to, File etag, Executor exe, IFileDownload progress) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (HttpRequest request = HttpRequest.get(url)
                .userAgent("CloudflaredForge Java Agent")) {
                ...
                if (to.isFile() && etag.isFile()
                    ...
                    && lines.get(0)
                        .equals(urlSha)
                    && lines.get(1)
                        .equals(Hashers.SHA1.hashifyHex(Files.readAllBytes(to.toPath())))) {
                    tmp.delete();
                ...
                try (IFileDownload ignore = progress) {
                    request.progress(progress)
                        .incrementTotalSize(request.contentLength())
                        .receive(tmp);
                    ...
                    if (tmp.renameTo(to)) {
                        Files.write(
                            etag.toPath(),
                            Arrays.asList(
                                (CharSequence[]) new String[] { urlSha,
    ...
    public static CompletableFuture<Integer> download(IGameProxy proxy) {
        ...
        if (architecture.getType() == OSArch.OSType.WINDOWS) {
            ...
            if (instructions == OSArch.InstructionSet.X86) {
                url = url + "cloudflared-windows-386.exe";
            } else {
                url = url + "cloudflared-windows-amd64.exe";
            }

            return download(
                url,
                new File(proxy.getExtraDataDir(), "cloudflared.exe"),
                etag,
                proxy.getBackgroundExecutor(),
                proxy.pushFileDownload());
        }

        if (architecture == OSArch.ArchDistro.GNU_LINUX) {

            String url = getLinuxUrl(baseUrl, instructions);
    ...
    private static String getLinuxUrl(String baseUrl, OSArch.InstructionSet instructions) {
        String url = baseUrl;
        if (instructions == OSArch.InstructionSet.X86) {
            url = url + "cloudflared-linux-386";
        } else if (instructions == OSArch.InstructionSet.X86_64) {
            url = url + "cloudflared-linux-amd64";
        } else if (instructions == OSArch.InstructionSet.ARM_32) {
            url = url + "cloudflared-linux-arm";
        } else if (instructions == OSArch.InstructionSet.ARM_64) {
            url = url + "cloudflared-linux-arm64";
        }
        return url;
    }
}
